...
interface IFaissVectorDatabaseGroup {
  /**
   * The document.
   */
  doc: IFaissVectorDatabaseDocument;
...
export class FaissVectorDatabaseConnector implements IVectorDatabaseConnector {
   ...
   */
  search(keywords: string[], vector: number[], k: number, minRelevance: number = 0, contentSize: number = 0): ISearchResult[] {
    ...
    const idGroups: IFaissVectorDatabaseGroup[] = [];
    let currentGroup: IFaissVectorDatabaseGroup | undefined;
    let currentVector: number[] | undefined;
    let currentLabel: number | undefined;
  ...
  private getCombinedScore(clusterVectors: number[][], centroid: number[], queryVector: number[], keywords: string[], content: string): number {
  ...
  private getCentroid(clusterVectors: number[][], queryVector: number[]): number[] {
  ...
  private keywordBoost(keywords: string[], content: string): number {
  ...
  private diversityAwareScore(clusterVectors: number[][], queryVector: number[]) {
    const similarities = clusterVectors.map((vec) => this.cosineSimilarity(vec, queryVector));

    const avgSimilarity = similarities.reduce((sum, sim) => sum + sim, 0) / similarities.length;
    const variance = similarities.reduce((sum, sim) => sum + (sim - avgSimilarity) ** 2, 0) / similarities.length;
